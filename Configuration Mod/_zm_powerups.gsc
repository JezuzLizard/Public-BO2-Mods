...
init() //checked matches cerberus output
{
	...
	set_zombie_var( "zombie_powerup_fire_sale_time", level.cmPowerupFireSaleDuration );
	...
	set_zombie_var( "zombie_powerup_insta_kill_time", level.cmPowerupInstaKillDuration, undefined, undefined, 1 );
	set_zombie_var( "zombie_powerup_point_doubler_on", 0, undefined, undefined, 1 );
	set_zombie_var( "zombie_powerup_point_doubler_time", level.cmPowerupDoublePointsDuration, undefined, undefined, 1 );
...
powerup_drop( drop_point ) //checked partially changed to match cerberus output
{
	...
	if ( rand_drop > level.cmPowerupRandomDropChance )
	{
...
start_fire_sale( item ) //checked matches cerberus output
{
	...
	{
		level.zombie_vars[ "zombie_powerup_fire_sale_time" ] += level.cmPowerupFireSaleDuration;
		return;
	}
...
start_carpenter( origin ) //checked partially changed to match cerberus output
{
	...
	for ( i = 0; i < players.size; i++ )
	{
		players[ i ] maps/mp/zombies/_zm_score::player_add_points( "carpenter_powerup", level.cmPowerupCarpenterPoints );
	}
	carp_ent delete();
}
...
powerup_timeout() //checked partially changed to match cerberus output
{
	...
	self show();
	wait_time = level.cmPowerupFieldLifetime;
...
nuke_powerup( drop_item, player_team ) //checked changed to match cerberus output
{
	...
	{
		if ( isDefined( level.cmPowerupNukeShouldWaitToKillZombies ) && level.cmPowerupNukeShouldWaitToKillZombies )
		{
			wait randomfloatrange( level.cmPowerupNukeMinTimeToKill, level.cmPowerupNukeMaxTimeToKill );
		}
	...
	for ( i = 0; i < players.size; i++ )
	{
		players[ i ] maps/mp/zombies/_zm_score::player_add_points( "nuke_powerup", level.cmPowerupNukePoints );
	}
}
...
double_points_powerup( drop_item, player ) //checked partially matches cerberus output did not change
{
	...
	level.zombie_vars[ team ][ "zombie_point_scalar" ] = level.cmPowerupDoublePointsScalar;
	players = get_players();
	...
	wait level.cmPowerupDoublePointsDuration;
...
full_ammo_powerup( drop_item, player ) //checked changed to match cerberus output
{
	...
	while ( i < players.size )
	{
		if ( players[ i ] maps/mp/zombies/_zm_laststand::player_is_in_laststand() && !level.cmPowerupMaxAmmoAffectsLaststandPlayers )
		{
			i++;
			continue;
		}
...
insta_kill_powerup( drop_item, player ) //checked matches cerberus output
{
	...
	wait level.cmPowerupInstaKillDuration;
...
insta_kill_on_hud( drop_item, player_team ) //checked matches cerberus output
{
	...
	{
		level.zombie_vars[ player_team ][ "zombie_powerup_insta_kill_time" ] = level.cmPowerupInstaKillDuration;
		return;
	}
...
point_doubler_on_hud( drop_item, player_team ) //checked matches cerberus output
{
	...
	{
		level.zombie_vars[ player_team ][ "zombie_powerup_point_doubler_time" ] = level.cmPowerupDoublePointsDuration;
		return;
	}
